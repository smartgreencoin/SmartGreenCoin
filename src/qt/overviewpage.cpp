...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SmartGreenCoin)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_SmartGreenCoin4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_SmartGreenCoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("SmartGreenCoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (chainActive.Height() >= 0 && chainActive.Height() < 11000000) {
    ui->label_lcolat->setText("450000 Coins");
    ui->label_mcolat->setText("550000 Coins");
    ui->label_fcolat->setText("650000 Coins");
  } else {
    ui->label_lcolat->setText("450000 Coins");
    ui->label_mcolat->setText("550000 Coins");
    ui->label_fcolat->setText("650000 Coins");
...
double BlockRewardSmartGreenCoin =  static_cast<double>(BlockReward/COIN);
//int64_t SmartGreenCoinSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardSmartGreenCoin));
//ui->label_SmartGreenCoinSupply_value->setText(QString::number(SmartGreenCoinSupply));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSMGRAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSMGRAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSMGRAmount = strAnonymizeSMGRAmount.remove(strAnonymizeSMGRAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSMGRAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSMGRAmount * COIN) nMaxToAnonymize = nAnonymizeSMGRAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSMGRAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
        //                                       .arg(strAnonymizeSMGRAmount));
        strAnonymizeSMGRAmount = strAnonymizeSMGRAmount.remove(strAnonymizeSMGRAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSMGRAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
