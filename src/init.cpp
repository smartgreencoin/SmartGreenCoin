...
#include "config/SmartGreenCoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("SmartGreenCoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "SmartGreenCoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "SmartGreenCoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 59235, 59237));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SmartGreenCoin/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SmartGreenCoin/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, SmartGreenCoin, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SmartGreenCoin/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SmartGreenCoin specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:59235"));
    ...
    strUsage += HelpMessageOpt("-anonymizeSmartGreenCoinamount=<n>", strprintf(_("Keep N SmartGreenCoin anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018-2019 The SmartGreenCoin Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("SmartGreenCoin-loadblk");
...
/** Sanity checks
 *  Ensure that SmartGreenCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SmartGreenCoin Core is shutting down."));
...
#endif
    // Make sure only a single SmartGreenCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SmartGreenCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SmartGreenCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SmartGreenCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SmartGreenCoin Core to complete") << "\n";
    ...
    nAnonymizeSMGRAmount = GetArg("-anonymizeSmartGreenCoinamount", 0);
    if (nAnonymizeSMGRAmount > 999999) nAnonymizeSMGRAmount = 999999;
    if (nAnonymizeSMGRAmount < 2) nAnonymizeSMGRAmount = 2;
    ...
    LogPrintf("Anonymize SmartGreenCoin Amount %d\n", nAnonymizeSMGRAmount);
    ...
    /* Denominations
       ...
       For example:
       1SmartGreenCoin+1000 == (.1SmartGreenCoin+100)*10
       10SmartGreenCoin+10000 == (1SmartGreenCoin+1000)*10
    */
