...
{
    if (blockHeight >= 0 && blockHeight < 11000000) {
      switch(vin_val) {
          case 450000 * COIN: return 1;
          case 550000 * COIN: return 2;
          case 650000 * COIN: return 3;
      }
    } else {
      switch(vin_val) {
          case 450000 * COIN: return 1;
...
*/
    ...
    // should be at least not earlier than block when 1000 SmartGreenCoin tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 SmartGreenCoin tx -> 1 confirmation
