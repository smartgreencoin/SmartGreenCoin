...
#if defined(NDEBUG)
#error "SmartGreenCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uSmartGreenCoin) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // Premine
      nSubsidy = 15000000000 * COIN;
    } else if (nHeight < 500) {    // POW phase
      nSubsidy = 50 * COIN;
    } else {    // POS phase
      nSubsidy = 9590 * COIN;
    }
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                if (nSpendHeight - coins->nHeight < Params().COINBASE_MATURITY()) {
                    if((nSpendHeight - coins->nHeight < 16))//fall through check for previous maturity
...
{
    RenameThread("SmartGreenCoin-scriptch");
